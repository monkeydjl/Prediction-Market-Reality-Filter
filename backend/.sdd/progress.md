...
**Post-merge fix HEAD:** `e78883b` (Reviews 1-3 below; last commit with code changes)
**Note:** Subsequent commits are ledger-only docs updates. Run `git log --oneline backend/.sdd/progress.md` for the latest ledger revision; run `git log --oneline -- backend/scripts/diagnose_event_quality.py backend/tests/test_diagnose_event_quality.py` for the latest code change.
...
- `e78883b` (14 commits since MERGE_BASE `6a13b71`); last commit with code changes. Ledger-only docs commits may follow — see header note.
