...
Migrated from app/services/world_cup_engines/world_cup_btd_model.py into the
sport-agnostic Kernel. This is a *faithful port*: the numerical computation
is identical to the original module so the two produce byte-identical output
and can be used interchangeably during the migration (see the equivalence
tests in tests/test_kernel_btd_model.py).
...
alpha is derived from Elo via the standard logit transform
alpha = 10 ** (elo / 400), so a 400-Elo gap produces a 10:1 strength ratio.

The gamma and home_advantage are fitted from historical international results
by scripts/fit_btd_model.py and persisted to data/btd_params.json. Knockout-
stage draw reduction is preserved as a multiplicative factor on gamma
