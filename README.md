...
- `WORLD_CUP_MATCH_SOURCE_URL` / `WORLD_CUP_MATCH_EVENTS_SOURCE_URL` / `WORLD_CUP_STANDINGS_SOURCE_URL` / `WORLD_CUP_PLAYER_AWARDS_SOURCE_URL` / `WORLD_CUP_PLAYER_STATUS_SOURCE_URL`：可选 raw feed URL；通过 bundle feeds preview/import 拉取后组装成多源 bundle。
- `WORLD_CUP_API_FOOTBALL_API_KEY`：可选 API-Football provider key；配置后可通过 bundle api-football preview/import 拉取 fixtures、standings、top scorers、injuries；`WORLD_CUP_API_FOOTBALL_FETCH_EVENTS=true` 时会额外按 fixture 拉取 card/event rows。
