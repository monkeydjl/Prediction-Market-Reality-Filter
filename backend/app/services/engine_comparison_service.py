...
def calculate_engine_accuracy():
    ...
    try:
        ...
        for match in finished_matches:
            # Get the last prediction for this match
            last_prediction = session.query(PredictionHistory).filter(
                PredictionHistory.match_id == match.match_id
            ).order_by(PredictionHistory.timestamp.desc()).first()

            if not last_prediction or not last_prediction.prediction_method:
                continue

            # Determine engine type
            method = last_prediction.prediction_method
